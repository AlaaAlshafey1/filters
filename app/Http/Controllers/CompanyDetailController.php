...
class CompanyDetailController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        /* ================= MAIN ABOUT + MEDIA ================= */
        $aboutData = array_merge($request->main ?? [], [
            'images' => $images,
    ...
    public function update(Request $request, CompanyDetail $companyDetail)
    {
        ...
        /* ================= MAIN DATA ================= */
        $data = $request->only(['title_ar', 'title_en', 'description_ar', 'description_en', 'video_url']);

        /* ================= IMAGES ================= */
        $images = $companyDetail->images ?? [];
        if ($request->has('deleted_images')) {
            foreach ($request->deleted_images as $delImg) {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $image) {
                $images[] = $image->store('about', 'public');
            }
        }
        $data['images'] = $images;
