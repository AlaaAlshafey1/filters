...
<div class="page-header py-3 px-3 mt-3 mb-3 bg-white shadow-sm rounded-3 border d-flex justify-content-between align-items-center flex-wrap gap-3" style="direction: rtl;">
    <div class="d-flex flex-column">
        <h4 class="content-title mb-1 fw-bold text-primary"><i class="bx bx-edit"></i> تعديل خدمة</h4>
        <small class="text-muted">قم بتحديث بيانات الخدمة</small>
    </div>
    <div>
        ...
        <div class="form-section mb-4">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="title1_ar" class="form-control" value="{{ $service->title1_ar }}" required>
                </div>

                <div class="col-md-6">
                    <label class="form-label">العنوان 1 (إنجليزي)</label>
                    <input type="text" name="title1_en" class="form-control" value="{{ $service->title1_en }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">العنوان 2 (عربي)</label>
                    <input type="text" name="title2_ar" class="form-control" value="{{ $service->title2_ar }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">العنوان 2 (إنجليزي)</label>
                    <input type="text" name="title2_en" class="form-control" value="{{ $service->title2_en }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">الوصف 1 (عربي)</label>
                    <textarea name="desc1_ar" class="form-control" rows="3">{{ $service->desc1_ar }}</textarea>
                </div>

                <div class="col-md-6">
                    <label class="form-label">الوصف 1 (إنجليزي)</label>
                    <textarea name="desc1_en" class="form-control" rows="3">{{ $service->desc1_en }}</textarea>
                </div>

                <div class="col-md-6">
                    <label class="form-label">العنوان 3 (عربي)</label>
                    <input type="text" name="title3_ar" class="form-control" value="{{ $service->title3_ar }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">العنوان 3 (إنجليزي)</label>
                    <input type="text" name="title3_en" class="form-control" value="{{ $service->title3_en }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">الوصف 2 (عربي)</label>
                    <textarea name="desc2_ar" class="form-control" rows="3">{{ $service->desc2_ar }}</textarea>
                </div>

                <div class="col-md-6">
                    <label class="form-label">الوصف 2 (إنجليزي)</label>
                    <textarea name="desc2_en" class="form-control" rows="3">{{ $service->desc2_en }}</textarea>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    @if($service->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/'.$service->image) }}" width="120" class="rounded border">
                        </div>
                    @endif
                ...
                <div class="col-md-12">
                    ...
                    <select name="is_active" class="form-select">
                        <option value="1" {{ $service->is_active ? 'selected' : '' }}>مفعل</option>
                        <option value="0" {{ !$service->is_active ? 'selected' : '' }}>غير مفعل</option>
                    </select>
                </div>
            </div>
        </div>
